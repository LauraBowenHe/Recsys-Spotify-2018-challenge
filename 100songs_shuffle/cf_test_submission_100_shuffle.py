...
pid_array = loadtxt(data_path + 'samples_100_shuffle_pids.txt.gz')
...
start = time()
pool = Pool(23)
results = pool.map(cf_predict_evaluation_slim_list, [index for index in test_list_pid])
pool.close()
pool.join()
...
savetxt('../submit/cf2submit_v10.txt', prediction_result, delimiter=',')
