...
test_listfname = '../data/train_list_info_title5songs.csv.gz'
#listfname = '../CFNLP/train_list_info_title10songs.csv.gz'
songfname = '../data/train_list_song_info.csv.gz'
nrows = 990000
testrows = 12000
threshould = 5

#df.util.get_train_test_comp('../train_list_info.csv.gz', songfname, nrows, threshould)
#util.get_train_test(listfname, songfname, nrows, testrows, threshould)
...
#with open('pid2followers.pkl', 'rb') as f:
#    pid2follower = pickle.load(f)

#cff_a = -0.5
#for pid, songs in u2s_v.items():
#    if cff_a == 0:
#        for song in songs:
#            mat[song, pid] = 1
#    else:
#        length = len(songs)
#        for song in songs:
#            mat[song, pid] =  math.pow(float(length), cff_a)

for song, pids in dic.items():
    for pid in pids:
        mat[song, pid] = 1
#        mat[song, pid] = math.pow(pid2follower[pid],coeff)
...
#coverage = np.array(test.apply(metric.coverage, axis=1).tolist())
#print("coverage is %s"%(np.mean(coverage)))

#test['cf_prob'] = scores.tolist()
#test.to_csv('cf2xgb_test1.csv.gz', compression='gzip')
with open("result.txt", 'a') as f:
    f.write("r_precision is %s \n"%(np.mean(r_precision_res)))
    f.write("ndcg is %s \n"%(np.mean(ndcg_res)))
    f.write("play click is %s \n"%(np.mean(playclick_res)))
#    f.write("coverage is %s \n"%(np.mean(coverage)))
    f.write("--------------------------\n")
test.to_csv('cf2xgb_5.csv.gz', index=False, compression='gzip')
